...
export default function AboutSection() {
  ...
  return (
    <section id="about" className="px-5 sm:px-8 lg:pl-32 py-20 relative z-10 border-t border-slate-900">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5">
            <Reveal delay={100}>
              <div className="glass-panel rounded-2xl border border-slate-800 bg-[#050a17] overflow-hidden shadow-xl">
                {/* Terminal Header */}
            ...
            <Reveal delay={150}>
              <div className="mt-5 glass-panel rounded-2xl p-5 border border-slate-800 bg-[#070d1d] flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-display font-bold text-sm text-white">{profile.degree}</h4>
                  <p className="text-xs text-cyan-300 mt-0.5">{profile.college}</p>
                  <p className="text-[11px] text-slate-500 font-mono mt-1">{profile.batch} · CGPA: {profile.cgpa} · New Delhi, India</p>
                </div>
              </div>
            </Reveal>
          </div>
            ...
            <Reveal delay={120}>
              <div className="glass-panel rounded-2xl p-6 sm:p-8 border border-slate-800 bg-[#070d1d]">
                ...
                <div className="grid sm:grid-cols-3 gap-4 pt-4 border-t border-slate-800/80">
                  <div className="bg-[#050a17] border border-slate-800 rounded-xl p-4">
                    <Code className="text-cyan-400 mb-2" size={20} />
                    <h4 className="font-display font-bold text-xs text-white mb-1">Full-Stack Web</h4>
                    <p className="text-[11px] text-slate-400 leading-normal">Building responsive interfaces with React/Next.js and FastAPI backends.</p>
                  </div>
                  <div className="bg-[#050a17] border border-slate-800 rounded-xl p-4">
                    <Cpu className="text-purple-400 mb-2" size={20} />
                    <h4 className="font-display font-bold text-xs text-white mb-1">AI &amp; RAG Systems</h4>
                    <p className="text-[11px] text-slate-400 leading-normal">Designing vector search pipelines with Qdrant, LangChain &amp; Gemini.</p>
                  </div>
                  <div className="bg-[#050a17] border border-slate-800 rounded-xl p-4">
                    <ShieldCheck className="text-emerald-400 mb-2" size={20} />
                    <h4 className="font-display font-bold text-xs text-white mb-1">Performance</h4>
                    <p className="text-[11px] text-slate-400 leading-normal">Optimizing API response times, database queries, and clean code.</p>
                  </div>
                </div>
              </div>
            </Reveal>
