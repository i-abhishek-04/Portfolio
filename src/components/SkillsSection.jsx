...
export default function SkillsSection() {
  ...
  return (
    <section id="skills" className="px-5 sm:px-8 lg:pl-32 py-20 relative z-10 border-t border-slate-900">
            ...
            {skillCategories.map((cat, idx) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2.5 px-5 py-3 rounded-xl border text-xs font-mono transition-all ${
                    activeCat === idx
                      ? "bg-blue-600/20 border-blue-500/60 text-white font-semibold shadow-[0_0_15px_rgba(56,189,248,0.25)]"
                      : "bg-[#060c1d] border-slate-800 text-slate-400 hover:text-slate-200"
                  }`}
                >
                  <IconComp size={16} className={activeCat === idx ? "text-cyan-400" : "text-slate-500"} />
                  {cat.title}
                </button>
              );
            })}
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {skillCategories[activeCat].skills.map((skill) => {
                ...
                return (
                  <div
                    key={skill.name}
                    className="bg-[#040814] border border-slate-800/90 rounded-xl p-4 flex items-center justify-between card-hover hover:border-cyan-500/50 transition-all group"
                  >
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-xl bg-slate-900/80 border border-slate-800 flex items-center justify-center group-hover:scale-105 transition-transform shrink-0">
                        {IconComp ? (
                    ...
                    <span className="text-[10px] font-mono text-cyan-400 bg-cyan-950/40 border border-cyan-500/30 px-2.5 py-1 rounded-lg shrink-0">
                      {skill.tag || "Core"}
                    </span>
                  </div>
                );
